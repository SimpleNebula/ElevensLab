...
public class CardGameGUI extends JFrame implements ActionListener {
	...
	public void repaint() {
		for (int k = 0; k < board.size(); k++) {
			...
			URL imageURL = null;
			// Try relative to this class's package
			imageURL = getClass().getResource(cardImageFileName);
			// If not found, try absolute from classpath root
			if (imageURL == null) {
				imageURL = getClass().getResource("/" + cardImageFileName);
			}
			// If still not found, try loading from filesystem path
			if (imageURL == null) {
				try {
					java.io.File f = new java.io.File(cardImageFileName);
					if (f.exists()) {
